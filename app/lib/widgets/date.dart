...
class DateInputField extends StatefulWidget {
  ...
  DateInputField(
      ...
      this.label})
      : firstDate =
            firstDate ?? DateTime.utc((initialDate?.year ?? DateTime.now().year) - 100, 1, 1, 0, 0),
        lastDate = lastDate ??
            DateTime.utc((initialDate?.year ?? DateTime.now().year) + 100, 12, 31, 23, 59),
        super(key: key);
...
class _DateInputFieldState extends State<DateInputField> {
    ...
    return TextField(
        ...
        decoration: InputDecoration(
            ...
            suffixIcon: currentDate == null
                ? null
                : IconButton(
                    icon: const Icon(PhosphorIcons.xLight), onPressed: () => changeDate(null))),
        readOnly: true,
        onTap: () {
  ...
  void changeDate(DateTime? nextDate) {
    ...
    Future.delayed(
        const Duration(milliseconds: 100), () => textFieldFocusNode.canRequestFocus = true);
    setState(() {
